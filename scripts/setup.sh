...
if $_ENV_REUSE; then
  ...
  if [ -n "$MLFLOW_TRACKING_URI" ]; then
    ...
    MLFLOW_EXPERIMENT_ID=${MLFLOW_EXPERIMENT_ID:-0}
    log_success "MLflow experiment: $MLFLOW_EXPERIMENT_ID"
  else
  ...
  # Prompt for A2A peer namespaces (for agent discovery)
  if [ "$A2A_ENABLED" = "true" ]; then
    log_info "A2A peer namespaces — other OpenClaw instances your agent can discover."
    log_info "  Your own namespace (${OPENCLAW_NAMESPACE}) is added automatically."
    log_info "  Example: bob-openclaw,alice-openclaw"
    read -p "  Peer namespaces (comma-separated, or Enter to skip): " A2A_PEER_NAMESPACES
    A2A_PEER_NAMESPACES="${A2A_PEER_NAMESPACES:-}"
    if [ -n "$A2A_PEER_NAMESPACES" ]; then
      log_success "A2A peers: $A2A_PEER_NAMESPACES"
    else
      log_info "No peers configured — agents can still reach peers by namespace if told"
    fi
    echo ""
  fi
...
# Derive TLS insecure from URI scheme (https = verify certs, http = no TLS)
...
# The base deployment already has sidecar.opentelemetry.io/inject: "openclaw-sidecar"
OTEL_SIDECAR_YAML="$REPO_ROOT/platform/observability/openclaw-otel-sidecar.yaml"
if [ -f "$OTEL_SIDECAR_YAML" ] && [ "${MLFLOW_TRACKING_URI:-}" != "" ]; then
  log_info "Deploying OTEL sidecar collector (MLflow: ${MLFLOW_TRACKING_URI}, experiment: ${MLFLOW_EXPERIMENT_ID})..."
  if $KUBECTL apply -f "$OTEL_SIDECAR_YAML" 2>/dev/null; then
    log_success "OTEL sidecar collector deployed"
  else
    log_warn "Could not deploy OTEL sidecar (OpenTelemetry Operator may not be installed)"
    log_warn "Install the operator, then run:"
    echo "    $KUBECTL apply -f $OTEL_SIDECAR_YAML"
  fi
else
...
SHADOWMAN_YAML="$REPO_ROOT/agents/openclaw/agents/shadowman/shadowman-base.yaml"
if [ -f "$SHADOWMAN_YAML" ]; then
  $KUBECTL apply -f "$SHADOWMAN_YAML"
  log_success "Default agent ConfigMap deployed"
else
  log_warn "shadowman-base.yaml not found — run envsubst or setup-agents.sh"
fi

# Install A2A skill ConfigMap only (NPS skill is deployed by setup-agents.sh with NPS agent)
if [ "$A2A_ENABLED" = "true" ]; then
  log_info "Installing A2A skill..."

  # Generate peer table for SKILL.md from A2A_PEER_NAMESPACES
  SKILL_MD="$REPO_ROOT/agents/openclaw/skills/a2a/SKILL.md"
  SKILL_TMP=$(mktemp)
  PEER_TABLE=""
  # Always include own namespace
  PEER_TABLE="| ${OPENCLAW_PREFIX} | ${OPENCLAW_NAMESPACE} | http://openclaw.${OPENCLAW_NAMESPACE}.svc.cluster.local:8080 |"
  ...
  if [ -n "${A2A_PEER_NAMESPACES:-}" ]; then
    ...
    for ns in "${PEERS[@]}"; do
      ...
      [ -z "$ns" ] && continue
      # Derive owner from namespace (strip -openclaw suffix)
      owner="${ns%-openclaw}"
      PEER_TABLE="${PEER_TABLE}
| ${owner} | ${ns} | http://openclaw.${ns}.svc.cluster.local:8080 |"
    done
  fi
  sed "s|{{OPENCLAW_REGISTRY}}|${PEER_TABLE}|" "$SKILL_MD" > "$SKILL_TMP"

  $KUBECTL create configmap a2a-skill \
    --from-file=SKILL.md="$SKILL_TMP" \
    -n "$OPENCLAW_NAMESPACE" --dry-run=client -o yaml \
    | $KUBECTL apply -f -
  rm -f "$SKILL_TMP"
  log_success "A2A skill ConfigMap deployed (${A2A_PEER_NAMESPACES:-no peers})"
fi
